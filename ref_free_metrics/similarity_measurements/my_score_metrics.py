...
def get_my_score(ref_token_vecs, ref_tokens, summ_token_vecs, summ_tokens, wmd_score_type, wmd_weight_type):
    recall_list = []
    precision_list = []
    f1_list = []
    empty_summs_ids = []
    ...
    for topic,docs,models in corpus_reader(year):
        ...
        if len(ref_dic) >= 15: #all(['ref' in rs for rs in ref_sources]):
            for rs in ref_sources:
                ...
                ref_sents_vecs.append([sent_vecs[k] for k in sorted(ref_dic.keys()) if ref_dic[k]['doc'] == rs])
        else:
            ref_sents.append([ref_dic[k]['text'] for k in sorted(ref_dic.keys())])
            ref_sents_vecs.append([sent_vecs[k] for k in sorted(ref_dic.keys())])
        # ref_vecs = []
        ref_tokens = []
        ...
        # get sbert-score
        pss = get_my_score(ref_sents_vecs, ref_tokens, summ_vecs, summ_tokens, sbert_type, wmd_weight_type)
        # compute correlation
